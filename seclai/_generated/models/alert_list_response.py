...
T = TypeVar("T", bound="AlertListResponse")


@_attrs_define
class AlertListResponse:
    """``GET /alerts`` — always the canonical ``{data, pagination}`` envelope.
    ...
    def from_dict(cls: type[T], src_dict: Mapping[str, Any]) -> T:
        ...
        for data_item_data in _data:
            data_item = AlertResponse.from_dict(data_item_data)

            data.append(data_item)

        pagination = PaginationResponse.from_dict(d.pop("pagination"))

        alert_list_response = cls(
            data=data,
            pagination=pagination,
        )

        alert_list_response.additional_properties = d
        return alert_list_response
